...
GO
/*
The column [MLInput].[TrainedModels].[Model_Name] is being dropped, data loss could occur.
*/

IF EXISTS (select top 1 1 from [MLInput].[TrainedModels])
    RAISERROR (N'Rows were detected. The schema update is terminating because data loss might occur.', 16, 127) WITH NOWAIT

GO
PRINT N'Dropping unnamed constraint on [MLInput].[TrainedModels]...';


GO
ALTER TABLE [MLInput].[TrainedModels] DROP CONSTRAINT [DF__TrainedMo__Model__66603565];
...
GO
ALTER TABLE [MLInput].[TrainedModels] DROP CONSTRAINT [DF__TrainedMo__Creat__6754599E];


GO
PRINT N'Starting rebuilding table [MLInput].[TrainedModels]...';
...
CREATE TABLE [MLInput].[tmp_ms_xx_TrainedModels] (
    [ModelName]   VARCHAR (128)   DEFAULT ('Default') NOT NULL,
    [Model]       VARBINARY (MAX) NOT NULL,
    [CreatedDate] DATE            DEFAULT (GetDate()) NOT NULL,
    PRIMARY KEY CLUSTERED ([ModelName] ASC)
);

IF EXISTS (SELECT TOP 1 1 
           FROM   [MLInput].[TrainedModels])
    BEGIN
        INSERT INTO [MLInput].[tmp_ms_xx_TrainedModels] ([Model], [CreatedDate])
        SELECT [Model],
               [CreatedDate]
        FROM   [MLInput].[TrainedModels];
    END

DROP TABLE [MLInput].[TrainedModels];

EXECUTE sp_rename N'[MLInput].[tmp_ms_xx_TrainedModels]', N'TrainedModels';
